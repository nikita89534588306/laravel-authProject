...
@section('content')
    ...
    @php  $LANG_CONST = 'messages.mainHeader.' @endphp
    <header class="flex items-center justify-between p-6">
        ...
        <div>
            <a href="{{ route('register') }}" class="rounded-md bg-green-600 py-2 px-4 font-semibold text-white shadow-lg transition duration-150 ease-in-out hover:bg-green-700 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">{{__($LANG_CONST.'btnRegister.textBtn')}}</a>
        </div>
    </header>
    <main class="flex flex-col justify-center p-6 pb-12">
        @include('auth.loginComponents.headerFormLogin')
        @include('auth.loginComponents.formLogin')
    </main>
